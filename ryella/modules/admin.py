...
from ryella.helpers import resize_image, get_user, get_text_content
...
async def _promote(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.promote_members:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.channels.EditAdminRequest(chat.id, user.id, types.ChatAdminRights(change_info=True, ban_users=True, delete_messages=True, pin_messages=True), rank=rank or "Admin"))
...
async def _demote(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.promote_members:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.channels.EditAdminRequest(chat.id, user.id, types.ChatAdminRights(change_info=None, ban_users=None, delete_messages=None, pin_messages=None), rank=rank or "member"))
...
async def _ban(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.ban_users:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.channels.EditBannedRequest(chat.id, user.id, types.ChatBannedRights(
            until_date=None,
            view_messages=True,
            send_messages=True,
            send_media=True,
...
async def _mute(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.restrict_members:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.channels.EditBannedRequest(chat.id, user.id, types.ChatBannedRights(until_date=None, send_messages=True)))
...
async def _unmute(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.restrict_members:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.channels.EditBannedRequest(chat.id, user.id, types.ChatBannedRights(until_date=None, send_messages=False)))
...
async def _pin(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.pin_messages:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.messages.UpdatePinnedMessageRequest(chat.id, msg.reply_to_msg_id, True))
...
async def _unpin(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.pin_messages:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.messages.UpdatePinnedMessageRequest(chat.id, msg.reply_to_msg_id, unpin=True))
...
async def _delall(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.delete_messages:
        return await msg.edit('`You are not an admin!`')
    try:
        await msg.client(functions.messages.DeleteHistoryRequest(chat.id, 0, revoke=True))
...
async def _admins(msg):
    ...
    if not chat.admin_rights or not chat.admin_rights.change_info:
        return await msg.edit('`You are not an admin!`')
    try:
        admins = await msg.client.get_participants(chat.id, filter=types.ChannelParticipantsAdmins)
        text = ''
        for admin in admins:
