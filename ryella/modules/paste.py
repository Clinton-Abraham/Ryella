    ...
    if r.status_code == 200:
        ...
        if data.get("bin"):
            ...
            caption = "<b>Bin Number:</b> <code>{}</code>\n<b>Bank:</b> <code>{}</code>\n<b>Country:</b> <code>{}</code>\n<b>Type:</b> <code>{}</code>\n<b>Brand:</b> <code>{}</code>".format(
                bin_number, bank, country, type, brand
            )
            await message.edit(caption, parse_mode="html", link_preview=False)
        else:
            await message.edit("No results found!")
...
async def sk(message):
    ...
    if "error" in resp:
        ...
        return await message.edit(
            msg.format(content, resp["error"]["message"], resp["error"]["type"]),
            parse_mode="html",
            link_preview=False,
        )
    else:
        ...
        if "error" in r:
            ...
            msg += "<b>Error Type:</b> <code>{}</code>\n"
            return await message.edit(
                msg.format(content, r["error"]["message"], r["error"]["type"]),
                parse_mode="html",
                link_preview=False,
            )
        else:
            ...
            total_amount = (
                [int(x["amount"]) for x in resp["data"]] if resp.get("data") else [0]
            )
            total_amount = str(sum(total_amount)) + " $"
            return await message.edit(
                msg.format(content, total_amount), parse_mode="html", link_preview=False
            )
